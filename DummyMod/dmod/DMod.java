    ...
    public void init(final ModLoaderEvents.ModInitEvent init) {
        ...
        TileType bottom = new TileType() {
            ...
            public void tick(TileWorld tileWorld, Region r, Tile tile, Time time) {
                Tile ab = tileWorld.get(tile.getGlobalTileX(), tile.getGlobalTileY() + 1);
                if (ab != null && tile.getType() != EMPTY) {
                    Region reg = tileWorld.getRegion(Region.toGlobalRegion(ab.getGlobalTileX()),
                            Region.toGlobalRegion(ab.getGlobalTileY()));
                    reg.setTile(new Tile(EMPTY, ab.getGlobalTileX(), ab.getGlobalTileY()));
                }
            }
        };
